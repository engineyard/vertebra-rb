...
describe Vertebra::Base64 do

  it 'should initialize with no errors when given correct parameters' do
    vb64 = Vertebra::Base64.new("abc")
    vb64.should be_a_kind_of(Vertebra::Base64)
    vb64 = Vertebra::Base64.new("abc",:dec)
    vb64.should be_a_kind_of(Vertebra::Base64)
    vb64 = Vertebra::Base64.new(["abc"].pack("m"),:enc)
    vb64.should be_a_kind_of(Vertebra::Base64)
  end

  it 'should raise an ArgumentError if the state is not :dec or :enc' do
    lambda { Vertebra::Base64.new("abc",:foo) }.should raise_error(ArgumentError)
  end
  ...
  it 'should encode a plain string when the Base64::encode method is called' do
    Vertebra::Base64.encode("abc").should == ["abc"].pack("m")
  end

  it 'should decode an encoded string when the Base64::decode method is called' do
    Vertebra::Base64.decode(["abc"].pack("m")).should == "abc"
  end

  it 'should cope gracefully if it gets something other than a string' do
    vb64 = Vertebra::Base64.new(123)
    vb64.decoded.should == "123"
    Vertebra::Base64.decode(Vertebra::Base64.encode(123)).should == "123"
  end
end


describe Vertebra::DateTime do
  ...
  it 'should initialize with an array of year, month, day, hour, minute, second' do
    ...
    vertebra_datetime = Vertebra::DateTime.new(*time_values)
    vertebra_datetime.should be_a_kind_of(Vertebra::DateTime)
  ...
  it 'should correctly check equality to another Vertebra::DateTime' do
    new_vertebra_datetime = Vertebra::DateTime.new(*time_now.to_a[0..5].reverse)
    new_and_different_datetime = Vertebra::DateTime.new(*Time.now.to_a[0..5].reverse)
